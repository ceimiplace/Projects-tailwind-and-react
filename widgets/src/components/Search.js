    ...
    <div className="search-container p-4 max-w-7xl mx-auto">
      <form onSubmit={(e) => e.preventDefault()} className="mb-4">
        ...
        <input
          ...
          className="block border-2 w-full border-slate-400 p-1"
          type="text"
        />
      </form>
      <div className="list-none">
        {results.map((elem) => (
          <div key={elem.pageid} className="p-2 bt-slate-400 border-2">
            <span className="font-bold">{elem.title}</span> <br />
            {removeSpan(elem.snippet)}...{" "}
            <button className="bg-slate-200 p-1 block rounded ml-auto">
              <a href={`https://en.wikipedia.org/?curid=${elem.pageid}`}>
                Read more
              </a>
            </button>
          </div>
